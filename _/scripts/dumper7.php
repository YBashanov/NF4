...
define('LIMIT', 1);
// mysql сервер - не всегда указываем порт, иногда он наоборот не нужен, как в примере с vps
...
if (!empty($_POST['login']) && isset($_POST['pass'])) {
	$link = mysqli_connect(DBHOST, $_POST['login'], $_POST['pass']);
	if ($link){
		setcookie("sxd", base64_encode("SKD101:{$_POST['login']}:{$_POST['pass']}"));
		header("Location: dumper7.php");
		mysqli_close($link);
		exit;
	}
	else {
...
elseif (!empty($_COOKIE['sxd'])) {
	...
	$link = @mysqli_connect(DBHOST, $user[1], $user[2]);
...
class dumper {
	...
	function __construct($link = null) {
		...
		if (file_exists(PATH . "dumper7.cfg.php")) {
		    include(PATH . "dumper7.cfg.php");
		}
		else{
		...
		preg_match("/^(\d+)\.(\d+)\.(\d+)/", mysqli_get_server_info($link), $m);
	...
	function backup() {
		...
		mysqli_select_db($this->link, $db) or trigger_error ("Не удается выбрать базу данных.<BR>" . mysqli_error($this->link), E_USER_ERROR);
		$tables = array();
		...
		$result = mysqli_query($this->link, "SHOW TABLE STATUS");
		$tabinfo = array();
		$tab_charset = array();
		$tab_type = array();
		$tabinfo[0] = 0;
		...
		$result = mysqli_query($this->link, "SET SQL_QUOTE_SHOW_CREATE = 1");
		// Кодировка соединения по умолчанию
		if ($this->mysql_version > 40101 && CHARSET != 'auto') {
			mysqli_query($this->link, "SET NAMES '" . CHARSET . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysql_error(), E_USER_ERROR);
        ...
        foreach ($tables AS $table){
			...
			if ($this->mysql_version > 40101 && $tab_charset[$table] != $last_charset) {
				if (CHARSET == 'auto') {
					mysqli_query($this->link, "SET NAMES '" . $tab_charset[$table] . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysql_error(), E_USER_ERROR);
            ...
            while(($result = mysqli_query($this->link, "SELECT * FROM `{$table}` LIMIT {$from}, {$limit}")) && ($total = mysqli_num_rows($result))){
            		while($row = mysqli_fetch_row($result)) {
						...
						for($k = 0; $k < $fields; $k++){
                    		if ($NumericColumn[$k])
                    			...
                    			$row[$k] = isset($row[$k]) ? "'" . mysqli_real_escape_string($this->link, $row[$k]) . "'" : "NULL";
	...
	function restore(){
		...
		echo tpl_l("Подключение к БД `{$db}`.");
		mysqli_select_db($this->link, $db) or trigger_error ("Не удается выбрать базу данных.<BR>" . mysql_error(), E_USER_ERROR);
		...
		if ($this->mysql_version > 40101 && (CHARSET != 'auto' || $this->forced_charset)) { // Кодировка по умолчанию, если в дампе не указана кодировка
			mysqli_query($this->link, "SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>" . mysqli_error($this->link), E_USER_ERROR);
			...
			if ($sql) {
			    if (preg_match("/;$/", $str)) {
						...
						elseif (preg_match("/CREATE TABLE/i", $sql)){
							...
							if (preg_match("/(CHARACTER SET|CHARSET)[=\s]+(\w+)/i", $sql, $charset)) {
								if (!$this->forced_charset && $charset[2] != $last_charset) {
									if (CHARSET == 'auto') {
										mysqli_query($this->link, "SET NAMES '" . $charset[2] . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>"
												. mysqli_error($this->link), E_USER_ERROR);
							...
							elseif(CHARSET == 'auto'){ // Вставляем кодировку для таблиц, если она не указана и установлена auto кодировка
								...
								if ($this->restore_charset != $last_charset) {
									mysqli_query($this->link, "SET NAMES '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>"
											. mysqli_error($this->link), E_USER_ERROR);
					...
					elseif($this->mysql_version > 40101 && empty($last_charset)) { // Устанавливаем кодировку на случай если отсутствует CREATE TABLE
						mysqli_query($this->link, "SET $this->restore_charset '" . $this->restore_charset . "'") or trigger_error ("Неудается изменить кодировку соединения.<BR>{$sql}<BR>"
								. mysqli_error($this->link), E_USER_ERROR);
						echo tpl_l("Установлена кодировка соединения `" . $this->restore_charset . "`.", C_WARNING);
						$last_charset = $this->restore_charset;
					}
    			...
    			if ($execute) {
            		$q++;
            		mysqli_query($this->link, $sql) or trigger_error ("Неправильный запрос.<BR>" . mysqli_error($this->link), E_USER_ERROR);
	...
	function main(){
		...
		$buttons = "<INPUT TYPE=submit VALUE=Применить><INPUT TYPE=button VALUE=Выход onClick=\"location.href = 'dumper7.php?reload'\">";
	...
	function db_select(){
		if (DBNAMES != '') {
			...
			foreach($items AS $item){
    			if (mysqli_select_db($this->link, $item)) {
    				$tables = mysqli_query($this->link, "SHOW TABLES");
    				if ($tables) {
    		...
    		while($item = mysqli_fetch_array($result)){
    			if (mysqli_select_db($this->link, $item[0])) {
    				$tables = mysqli_query($this->link, "SHOW TABLES");
    				if ($tables) {
    	  			    $tabs = mysqli_num_rows($tables);
	...
	function fn_save(){
		if (SC) {
			$ne = !file_exists(PATH . "dumper7.cfg.php");
		    $fp = fopen(PATH . "dumper7.cfg.php", "wb");
        	fwrite($fp, "<?php\n\$this->SET = " . fn_arr2str($this->SET) . "\n?>");
        	fclose($fp);
			if ($ne) @chmod(PATH . "dumper7.cfg.php", 0666);
			$this->fn_index();
		}
	}
...
<BODY BGCOLOR=#ECE9D8 TEXT=#000000 STYLE="font-family: verdana, arial;font-size: 13px;">
...
<B><A HREF=http://sypex.net/products/dumper/ STYLE="color: white; text-decoration: none;">Dumper PHP7</A></B><IMG ID=GS WIDTH=1 HEIGHT=1 STYLE="visibility: hidden;"></TD>
</TR>
<TR>
<FORM NAME=skb METHOD=POST ACTION=dumper7.php>
...
<TABLE WIDTH=100% BORDER=0 CELLSPACING=0 CELLPADDING=2>
<TR>
<TD>Для работы Sypex Dumper Lite требуется:<BR> - PHP7, Internet Explorer 5.5+, Mozilla либо Opera 8+ (<SPAN ID=sie>-</SPAN>)<BR> - включено выполнение JavaScript скриптов (<SPAN ID=sjs>-</SPAN>)</TD>
</TR>
</TABLE>
</FIELDSET>
</SPAN>
